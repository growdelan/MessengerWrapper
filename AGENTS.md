...
- Licznik nieprzeczytanych jest wykrywany event-driven przez wstrzyknięty `WKUserScript` (obserwacja zmian tytułu/DOM przez `MutationObserver`) i wysyłany do natywnego kodu przez `WKScriptMessageHandler`, a następnie rozsyłany przez `NotificationCenter` (`messengerWrapper.unreadCountDidChange`) do UI status bar oraz Dock badge.
